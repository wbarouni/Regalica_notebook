...
# Find files to scan
FILES=$(find . -type f \( -name "*.js" -o -name "*.json" -o -name "*.html" -o -name "*.css" -o -name "*.scss" -o -name "*.md" -o -name "*.yaml" -o -name "*.yml" -o -name "*.ts" -o -name "*.tsx" -o -name "*.jsx" \) | grep -vE "($IGNORE_DIRS)" | grep -vE "($IGNORE_FILES)" | grep -v "./scripts/" || true)
